...
class Alert(object):
    ...
    def __repr__(self):
        return "<Alert for {} with users {} on price {}>".format(self.user_email, self.item.name, self.price_limit)
    ...
    def send(self):
        return requests.post(
            ...
            data={
                ...
                "text": "We have found a deal! ({}). Visit {}, in case of navigating to the alerts".format(
                    self.item.url,
                    "http://vipps-price-tracker.com/alerts/{}".format(self._id))
            }
        )
